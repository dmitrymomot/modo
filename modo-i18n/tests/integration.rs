...
use http::Request;
use modo_i18n::extractor::ResolvedLang;
use modo_i18n::t;
...
fn setup(
    ...
    std::sync::Arc<modo_i18n::TranslationStore>,
    std::path::PathBuf,
) {
...
fn middleware_app(store: Arc<modo_i18n::TranslationStore>) -> Router {
    Router::new()
        .route("/", get(lang_handler))
        .layer(modo_i18n::layer(store))
}

fn middleware_app_with_source(
    store: Arc<modo_i18n::TranslationStore>,
    source: impl Fn(&http::request::Parts) -> Option<String> + Send + Sync + 'static,
) -> Router {
    Router::new()
        .route("/", get(lang_handler))
        .layer(modo_i18n::layer_with_source(store, source))
}
