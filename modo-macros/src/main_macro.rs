            ...
            modo::tokio::runtime::Builder::new_multi_thread()
                ...
                .block_on(async {
                    ...
                    let _sentry_guard = config.sentry_dsn.as_deref().map(|dsn| {
                        modo::sentry::init((dsn, modo::sentry::ClientOptions {
                            traces_sample_rate: match config.environment {
                                modo::config::Environment::Production => 0.1,
                                _ => 1.0,
                            },
                            environment: Some(match config.environment {
                                modo::config::Environment::Production => "production".into(),
                                modo::config::Environment::Development => "development".into(),
                                modo::config::Environment::Test => "test".into(),
                            }),
                            release: modo::sentry::release_name!(),
                            ..Default::default()
                        }))
                    });
                    ...
                    if _sentry_guard.is_some() {
                        ...
                        modo::tracing_subscriber::registry()
                            .with(modo::tracing_subscriber::fmt::layer().with_filter(stdout_filter))
                            .with(modo::sentry::integrations::tracing::layer().with_filter(sentry_filter))
                            .init();
                    ...
                    let #app_ident = modo::app::AppBuilder::new(config);
