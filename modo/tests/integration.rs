...
mod session_integration {
    ...
    use modo::extractors::auth::{Auth, OptionalAuth, UserProvider, UserProviderService};
    use modo::session::{
    ...
    impl SessionStore for MemoryStore {
        ...
        async fn touch(&self, _id: &SessionId, _ttl: std::time::Duration) -> Result<(), Error> {
            Ok(())
        }
        ...
        async fn read_by_token(&self, token: &SessionToken) -> Result<Option<SessionData>, Error> {
            Ok(self
                .sessions
                .lock()
                .unwrap()
    ...
    impl SessionStore for FailingReadStore {
        async fn create(&self, _user_id: &str, _meta: &SessionMeta) -> Result<SessionId, Error> {
            unimplemented!()
        }
        async fn create_with(
            &self,
        ...
        async fn read(&self, _id: &SessionId) -> Result<Option<SessionData>, Error> {
            Err(Error::internal("simulated DB failure"))
        }
        async fn touch(&self, _id: &SessionId, _ttl: std::time::Duration) -> Result<(), Error> {
            unimplemented!()
        }
        ...
        async fn destroy_all_for_user(&self, _user_id: &str) -> Result<(), Error> {
            unimplemented!()
        }
        async fn read_by_token(&self, _token: &SessionToken) -> Result<Option<SessionData>, Error> {
            Err(Error::internal("simulated DB failure"))
        }
        async fn update_token(
            &self,
    ...
    impl UserProvider for TestUserProvider {
        type User = TestUser;
        async fn find_by_id(&self, id: &str) -> Result<Option<TestUser>, Error> {
            match id {
    ...
    const COOKIE_NAME: &str = "_session";
    ...
    async fn session_transient_db_error_preserves_cookie() {
        ...
        let config = modo::config::AppConfig::default();
        let state = AppState {
            db: None,
            services: Default::default(),
            config,
