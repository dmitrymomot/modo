...
mod tests {
    ...
    fn test_config(dir: &std::path::Path) -> crate::templates::TemplateConfig {
        crate::templates::TemplateConfig {
    ...
    fn render_simple_template() {
        ...
        let engine = super::engine(&test_config(&dir)).unwrap();
    ...
    fn template_not_found_error() {
        ...
        assert!(matches!(
            result,
            Err(crate::templates::TemplateError::NotFound { .. })
        ));
    ...
    fn env_mut_accessible_during_setup() {
        ...
        let mut engine = super::engine(&test_config(&dir)).unwrap();
        engine
            .env_mut()
