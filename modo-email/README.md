...
access it via the `Service<T>` parameter pattern generated by the `#[job]` macro.
...
// Enqueue from an HTTP handler (JobQueue is an axum extractor):
async fn signup_handler(queue: JobQueue) -> Result<(), modo::Error> {
    let payload = SendEmailPayload::from(
        SendEmail::new("welcome", "user@example.com")
            .var("name", "Alice"),
    );
    queue.enqueue("send_email", &payload).await?;
    Ok(())
}
...
`Service<T>` here is `modo::extractor::service::Service<T>`, re-exported and wired by the
`#[job]` macro. Add it to your imports as needed.
...
EmailTemplate {
    ...
    layout: Some("default".to_string()),  // None uses the built-in default
}
```
