...
`DbPool` and queries via the Record trait. Use `User::query().filter().one()` which returns
`Option<T>` — do not use `User::find_by_id()` here because it returns `Result<T, Error>`
with auto-404, which would turn a missing user into an error instead of `None`.
