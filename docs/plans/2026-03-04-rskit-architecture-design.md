# rskit — Architecture Document

## Context

Building a Rust web framework for micro-SaaS applications, inspired by Go's [forge](https://github.com/dmitrymomot/forge). Where forge embraces "no magic" with explicit wiring, rskit embraces **"full magic" through Rust's compile-time capabilities** — proc macros, derives, generics, trait system — with zero runtime cost.
...
3. **Convention over configuration** — sensible defaults; `#[rskit::main]` wires the entire app automatically
...
Single crate with feature flags + mandatory `rskit-macros` (proc macros must be separate in Rust).
...
rskit/
  ...
  rskit/                    # Main crate
    ...
    src/
      ...
      error.rs              # RskitError, content negotiation, IntoResponse
      middleware/
...
1. `#[rskit::main]` expands, collects all auto-discovered routes/jobs/modules via `inventory`
...
Loaded from env vars with `RSKIT_` prefix, then `.env` file, then defaults.
...
inventory::submit! {
    rskit::RouteRegistration {
        method: rskit::Method::GET,
        path: "/users/:id",
...
Framework provides `RskitError` base enum for common cases. Users define their own domain errors.
...
- `rskit-macros`: `#[handler]`, `#[rskit::main]`
- `inventory`-based auto-discovery
- `AppBuilder` with config, graceful shutdown
- `Db` extractor, SeaORM v2 + SQLite + WAL mode
- `RskitError` with content negotiation
- `Service<T>` extractor

**Milestone:** `#[handler(GET, "/")] async fn index() -> &'static str { "Hello rskit" }` works with `#[rskit::main]`.
